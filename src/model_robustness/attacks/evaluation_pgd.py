...
def evaluate_images(tune_config):
    ...
    if tune_config["dataset"] == "MNIST":
        ...
        old_result_path = os.path.join(old_result_path, "CIFAR10", "small")
        if tune_config["setup"] == "hyp-10-r":
            old_result_path = os.path.join(old_result_path, "tune_zoo_cifar10_small_hyperparameter_10_random_seeds")
        elif tune_config["setup"] == "hyp-10-f":
            old_result_path = os.path.join(old_result_path, "tune_zoo_cifar10_small_hyperparameter_10_fixed_seeds")
        elif tune_config["setup"] == "seed":
            old_result_path = os.path.join(old_result_path, "tune_zoo_cifar10_uniform_small")

    data_path = os.path.join(data_root, tune_config["dataset"], "PGD",
        tune_config["setup"], f"eps_{tune_config['eps_iter']}", "perturbed_dataset.pt")
    ...
    for i, path in enumerate(model_paths):
        ...
        save_results_path = Path(
            os.path.join(data_root, tune_config["dataset"], "PGD", 
            tune_config["setup"], f"eps_{tune_config['eps_iter']}", "results", path)
        )
