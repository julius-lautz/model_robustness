...
from attacks.networks import ConvNetLarge, ConvNetSmall, ResNet18
...
def load_zoo(zoo_name, args):

    checkpoint_path = Path(args.path_to_zoos["resnet"][zoo_name])
    data_path = checkpoint_path.joinpath("dataset.pt")
    zoo_path = args.path_to_zoo_results["resnet"][zoo_name]

    dataset = torch.load(data_path)["testset"]
    #zoo = torch.load(zoo_path)

    return dataset, checkpoint_path


def get_best_model_path(zoo_name, args):
    ...
    checkpoint_path = args.path_to_zoos["resnet"][zoo_name]
    ...
    _logger.debug(f"best model has accuracy = {best_acc}")
...
def main(args):
    ...
    for ds in args.zoos["resnet"]:

        config["dataset"] = ds

        # Get dataset + zoo
        _logger.info("Getting zoo")
        dataset, checkpoint_path = load_zoo(config["dataset"], args)

        # Get the best performing model from zoo
        _logger.info("Getting best model from zoo")
        path_list, max_index, best_model_path = get_best_model_path(config["dataset"], args)
        ...
        with open((saving_path.joinpath(f"configs/resnet_{config['dataset']}_config.json")), "w") as f:
        ...
        results_df.to_csv(saving_path.joinpath(f"dataframes/resnet_{config['dataset']}_df.csv"))
